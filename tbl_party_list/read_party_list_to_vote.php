...
$check_status=false;
// tbl_Voting_Program.voting_status = \"Opened\"
...
WHERE tbl_Voting_Program.voting_program_id=$voting_program_id
...
while ($CandidatePosition = mysqli_fetch_array($CandidatePositions)) {
	...
	while ($PartyList = mysqli_fetch_array($PartyLists)) {
		...
		if($counter==1){
			echo "<div class='col-lg-12 alert alert-success' style='text-align:center;font-weight:bold; color:black;' width='100%;'><h5><b>{$CandidatePosition['candidate_position_name']}</b></h5></div>";
		}
